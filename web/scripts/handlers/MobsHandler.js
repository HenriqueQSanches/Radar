...
export class MobsHandler {
    constructor() {
        this.mobsList = [];
        this.mistList = [];
        this._lastCalibrationDelta = null;
    }
    ...
    AddEnemy(id, typeId, posX, posY, healthNormalized, maxHealth, enchant, rarity) {
        ...
        // maxHealth anchors the lookup against game-data drift (see MobsDatabase.getMobInfo)
        const dbInfo = window.mobsDatabase?.getMobInfo(typeId, maxHealth, (enchant || 0) > 0);
        this._reidentifyIfDrifted();
        const hasKnownInfo = this._applyDbInfo(mob, dbInfo);
    ...
    /**
     * Assign database identity (type, name, tier, category) to a mob.
     * Also used to re-label mobs already in the list when the offset
     * calibration converges on a new delta.
     * @param {Mob} mob
     * @param {Object|null} dbInfo - Result of MobsDatabase.getMobInfo
     * @returns {boolean} Whether the mob was identified
     * @private
     */
    _applyDbInfo(mob, dbInfo) {
        mob.type = EnemyType.Enemy;
        mob.name = null;
        mob.tier = 0;
        mob.uniqueName = undefined;
        mob.category = null;
        mob.namelocatag = null;
        let hasKnownInfo = false;

        if (dbInfo && dbInfo.isHarvestable) {
            // dbInfo.tier is the Loot.Harvestable tier — the one the marking filter matches on
            mob.tier = dbInfo.tier || getLivingHarvestTier({
                u: dbInfo.uniqueName,
                t: dbInfo.combatTier,
                l: dbInfo.lootType,
            }) || 0;
        ...
        if (mob.type === EnemyType.LivingHarvestable || mob.type === EnemyType.LivingSkinnable) {
            mob.enchantmentLevel = this.calculateEnchantment(mob.enchantmentLevel);
        }
    ...
    /**
     * Re-run database identification for every tracked mob after the
     * MobsDatabase offset calibration adopts a new delta, so mobs spawned
     * before convergence get correct labels too.
     * @private
     */
    _reidentifyIfDrifted() {
        const db = window.mobsDatabase;
        const delta = db?.calibrationDelta;
        if (typeof delta !== 'number' || delta === this._lastCalibrationDelta) return;
        this._lastCalibrationDelta = delta;
        if (this.mobsList.length === 0) return;

        for (const mob of this.mobsList) {
            const dbInfo = db.getMobInfo(mob.typeId, mob.maxHealth, (mob.enchantmentLevel || 0) > 0);
            this._applyDbInfo(mob, dbInfo);
        }
        window.logger?.info(CATEGORIES.MOBS, 'mobs_reidentified_after_offset_drift', {
            delta,
            count: this.mobsList.length
        });
    }
    ...
    updateEnchantEvent(parameters) {
        ...
        // Enchant is always 0-4; anything else is a shifted/foreign payload — ignore it
    ...
    cleanupStaleEntities(maxAgeMs = 120000) {
        ...
        // Living resources are stationary and never receive position/health updates,
        // so age says nothing about them; they leave via death/Leave events or map change.
        this.mobsList = this.mobsList.filter(entity =>
            entity.type === EnemyType.LivingHarvestable ||
            entity.type === EnemyType.LivingSkinnable ||
