...
export class PlayersHandler {
    constructor() {
        ...
        this.alertedThreatIds = new Set();
        this._audioCtx = null;
        this._soundBuffer = null;
        this._soundBufferLoading = null;
    ...
    playThreatSound() {
        const boost = settingsSync.getNumber('settingSoundBoost', 6);
        try {
            if (this._playBoostedSound(boost)) return;
        } catch (err) {
            window.logger?.debug(CATEGORIES.PLAYERS, 'audio_boost_error', {error: err?.message});
        }
    ...
    _playBoostedSound(boost) {
        ...
        if (!this._soundBuffer) {
            if (!this._soundBufferLoading) {
                this._soundBufferLoading = fetch('/sounds/player.mp3')
                    .then(r => r.arrayBuffer())
                    .then(b => ctx.decodeAudioData(b))
                    .then(buf => { this._soundBuffer = buf; })
                    .catch(err => {
                        this._soundBufferLoading = null;
                        window.logger?.debug(CATEGORIES.PLAYERS, 'audio_decode_error', {error: err?.message});
                    });
            }
            return false;
        }
        ...
        src.buffer = this._soundBuffer;
        const gain = ctx.createGain();
        gain.gain.value = Math.max(1, Number(boost) || 1);
        src.connect(gain).connect(ctx.destination);
        src.start(0);
        return true;
    }

    _emitThreatAlert(player) {
    ...
    removePlayer(id) {
        ...
        this.alertedThreatIds.delete(id);
    }
