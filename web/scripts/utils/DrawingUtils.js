...
export class DrawingUtils {
    ...
    interpolateEntity(entity, lpX, lpY, t) {
        ...
        // A non-finite local-player position (e.g. mid map-change) would poison
        // hX/hY permanently — lerp(NaN, x) stays NaN and the snap guard below
        // only fires on exact zero. Skip the frame instead.
        if (!Number.isFinite(hX) || !Number.isFinite(hY)) return;

        if ((entity.hY === 0 && entity.hX === 0) ||
            !Number.isFinite(entity.hX) || !Number.isFinite(entity.hY)) {
            entity.hX = hX;
            entity.hY = hY;
        }
