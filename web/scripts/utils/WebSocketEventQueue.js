...
// rAF stalls in hidden/background tabs; this timer keeps events flowing there.
...
export class WebSocketEventQueue {
    constructor() {
        ...
        this.timeoutId = null;  // Backstop timer for hidden tabs
    ...
    queueEventInternal(messageType, params) {
        if (messageType !== 'event') {
            // Requests/responses (e.g. ChangeCluster/Join, which clear all handlers)
            // must not overtake events already received: drain the queue first so
            // wire order is preserved and old-map spawns can't be replayed after a clear.
            this.flush();
    ...
    scheduleFlush() {
        ...
        this.timeoutId = setTimeout(() => this.flush(), FLUSH_BACKSTOP_MS);
    }
